<?
    connect();
    $q = mysql_query("SELECT * FROM walls_games WHERE id=" . $_GET['id']);
    ...
    $players = array();
    $q = mysql_query("SELECT * FROM walls_players WHERE game_id=" . $_GET['id']);
    while ($row = mysql_fetch_array($q)) {
      if ($row['team']=="red")
    ...
    $kills = array();
    $q = mysql_query("SELECT * FROM kills WHERE server='walls' AND game_id=" . $_GET['id'] . " ORDER BY date DESC");
    while ($row = mysql_fetch_array($q)) {
      $kill = array();
...
<? foreach ($players as $username => $p) { ?>
  <?
    $q = mysql_query("SELECT * FROM kills WHERE server='walls' AND target='" . $username . "' AND game_id=" . $_GET['id']);
  ...
  <tr>
    ...
    <td><?= ucwords($p['kit']) ?></td>
    <td><span class="tip" title="<?= implode(', ',$p['kills_string']) ?>"><?= $p['kills'] ?></span></td>
    <td><?

<?
    connect();
    $q = mysql_query("SELECT * FROM hg_games WHERE id=" . $_GET['id']);
    ...
    $players = array();
    $q = mysql_query("SELECT * FROM hg_players WHERE game_id=" . $_GET['id']);
    while ($row = mysql_fetch_array($q)) {
      $arr = array();
      $arr['kit'] = $row['kit'];
      if ($row['winer'] == 1)
    ...
    $kills = array();
    $q = mysql_query("SELECT * FROM kills WHERE server='hg' AND game_id=" . $_GET['id'] . " ORDER BY date DESC");
    while ($row = mysql_fetch_array($q)) {
      $kill = array();

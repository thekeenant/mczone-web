...
<? $title = $_GET['category'] ?>
<? connect(); ?>
<? $q = mysql_query("SELECT * FROM posts WHERE title IS NOT NULL AND category LIKE '$category' AND title!='' ORDER BY sticky DESC,updated DESC"); ?>
<div class="page-header">
	<h1><? echo ($category == null) ? "Latest Topics" : $category ?></h1>
</div>
<ul class="breadcrumb">
...
<?php
	include('connect.php');
	$where = "title IS NOT NULL AND category LIKE '$category' AND title!=''";
	...
	if ($private) {
		if (getPosition($username) != "admin" && getPosition($username) != "mod")
			$where .= " AND (author='$username' OR sticky=1)";
	}
	$tableName="posts";		
	$targetpage = "category?category=$category&"; 	
	$limit = 20;
	$query = "SELECT COUNT(*) as num FROM $tableName WHERE $where ORDER BY sticky DESC,updated DESC";
    ...
    // Get page data
	$query1 = "SELECT * FROM $tableName WHERE $where ORDER BY sticky DESC, updated DESC LIMIT $start, $limit";
  ...
  while ($row2 = mysql_fetch_array($q2)) {
    ...
    if ($row['deleted'] == 1) {
    	...
    	if ($row['author'] == $username)
    		...
    		$exit = false;
    	else if (getPosition($username) == "mod")
    		$exit = false;
    }
  }
  if ($exit)
...
<? if ($private) { ?>
	<hr />
	<center><small>Note: This category is private. You can only view your created topics</small></center>
<? } ?>
<?    
if ($category != null && loggedIn()) {
  if (!in_array($category, adminCategories()) || $username=="funkystudios") { ?>
